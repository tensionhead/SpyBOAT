...
def show_vfield(vx, vy, skip=5, vlen_perc=None, color="k", width=0.0025):
...
def show_phase_gradient(frame,
                        skip=5,
                        vlen_perc=99,
                        plevels=15,
                        pretty_ticks=False):

    pdx, pdy, _ = gf.get_masked_gradient(frame, mask_value=-1)
    ...
    # ax.contour(frame, levels=15, cmap='gray_r')
    cb = ppl.colorbar(im, ax=ax, shrink=0.75, orientation="horizontal", pad=0.02)
    cb.set_label('phase')
    if pretty_ticks:
        cb.set_ticks([0, pi / 2, pi, 3*pi / 2, 2*pi])
...
def show_flux_grad(frame, k=15, vlen_perc=99, flux_perc=98, skip=10, show_masked=True):
